...
void Sir_model::Board::draw(int& refresh_rate) {
    float bit_size = 1.;
   ...
   /* if (dimension_ < 100) {
    ...
    sf::RenderWindow window(sf::VideoMode(win_size + static_cast<int>((2 * win_size / 3)), win_size),
        "LEFT WINDOW DESEASE RIGHT-DOWN QUARANTENE  RIGHT-UP-FREE", sf::Style::Close | sf::Style::Resize);
    ...
    while (window.isOpen())
    {
        ...
        // questo if dovr� comprendere le funzioni avanzate di fusco.La funzione evolve,move e airplane devono attivarsi a seconde dell unum che gli passiamo.
        ...
        if (time.asMilliseconds() > refresh_rate) {
            ...
            // questo � il grafico che si attiva solo se la mia opzione( l'ultima) � attiva.
            if ((int)advanced_opt>=4) {
                ...
                int punto_grafico_infetto = static_cast<int>((counter.num_i-q_counter.num_i)*0.5 /proportion);
                int punto_grafico_sano =static_cast<int>((counter.num_s-q_counter.num_s)*0.5/(proportion));
                int punto_grafico_ricoverato = static_cast<int>((counter.num_r-q_counter.num_r)*0.5 /(proportion));
                grafico_in_quarantene.push_back({ q_point_s,q_point_i,q_point_r });
                grafico_out_quarantene.push_back({ punto_grafico_sano , punto_grafico_infetto , punto_grafico_ricoverato });
                ...
                // questo grafico si attiva in tutti gli altri casi.
                    ...
                    int punto_grafico_infetto = static_cast<int>(counter.num_i / dimension_);
                    int punto_grafico_sano = static_cast<int>(counter.num_s / dimension_);
                    int punto_grafico_ricoverato = static_cast<int>(counter.num_r / dimension_);
