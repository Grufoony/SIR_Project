                ...
                for (int i = dimension_ + 1; i < (5 * dimension_  / 3); ++i) {
                    ...
                    for (int counter = 0; counter != (int)graph_out_quarantine_.size(); ++counter) {
                        if (graph_out_quarantine_[counter].num_i <= 1) {
                            inf_bit.setPosition(static_cast<float>(dimension_ + counter)*bit_size, static_cast<float>(dimension_ / 2)*bit_size - 2);
                            window.draw(inf_bit);
                        }
                        else {
                            inf_bit.setPosition(static_cast<float>(dimension_ + counter + 1)*bit_size,bit_size* static_cast<float>(dimension_ / 2 - graph_out_quarantine_[counter].num_i) - 2);
                            window.draw(inf_bit);
                        }
