...
void sir::Board::draw(int &millisecondi) {
  ...
  sf::RenderWindow window(
      sf::VideoMode(win_size + static_cast<int>((2 * win_size / 3)), win_size),
  ...
  while (window.isOpen()) {
    ...
    if ((int)advanced_opt_ >= 4) {
      ...
      graph_points.num_s = static_cast<int>(
          (counter_.num_s - q_counter_.num_s) * 0.5 / (proportion));
      graph_points.num_i = static_cast<int>(
          (counter_.num_i + counter_.num_q - q_counter_.num_i) * 0.5 /
          (proportion));
      graph_points.num_r = static_cast<int>(
          (counter_.num_r - q_counter_.num_r) * 0.5 / (proportion));
        ...
        for (int counter = 0; counter != (int)graph_out_quarantine_.size();
          ...
          if (graph_out_quarantine_[counter].num_i <= 1) {
            inf_bit.setPosition(
                ...
                static_cast<float>(dimension_ / 2) * bit_size - 3);
            window.draw(inf_bit);
          } else {
            inf_bit.setPosition(
          ...
          if (graph_in_quarantine_[counter].num_i <= 1) {
            inf_bit.setPosition(bit_size *
                                    static_cast<float>(dimension_ + counter),
                                bit_size * static_cast<float>(dimension_) - 2);
            window.draw(inf_bit);
          } else {
            inf_bit.setPosition(
