         ...
         std::cout << "" << day_ + 1 << "            " << counter_.num_i + q_counter_.num_i << '\n';
            ...
            if ((int)advanced_opt_ >= 4) {
                ...
                for (int i = dimension_ + 1; i < (5 * dimension_  / 3); ++i) {
                    board.setPosition(i * bit_size,bit_size* static_cast<float>(dimension_ / 2));
                    window.draw(board);
                    for (int counter = 0; counter != (int)graph_out_quarantine_.size(); ++counter) {
                        if (graph_out_quarantine_[counter].num_i <= 1) {
                            inf_bit.setPosition(static_cast<float>(dimension_ + counter)*bit_size, static_cast<float>(dimension_ / 2)*bit_size - 3);
                            window.draw(inf_bit);
                        }
                        else {
                            inf_bit.setPosition(static_cast<float>(dimension_ + counter + 1)*bit_size,bit_size* static_cast<float>(dimension_ / 2 - graph_out_quarantine_[counter].num_i) - 3);
                            window.draw(inf_bit);
                        }
                        if (graph_out_quarantine_[counter].num_s <= 1) {
                            sus_bit.setPosition(bit_size*static_cast<float>(dimension_ + counter),bit_size* static_cast<float>(dimension_ / 2) - 1);
                            window.draw(sus_bit);
                        }
                        else {
                            sus_bit.setPosition(bit_size*static_cast<float>(dimension_ + counter + 1),bit_size* static_cast<float>(dimension_ / 2 - graph_out_quarantine_[counter].num_s) + 1);
                            window.draw(sus_bit);
                        }
                        if (graph_out_quarantine_[counter].num_r <= 1) {
                            rec_bit.setPosition(bit_size*static_cast<float>(dimension_ + counter),bit_size* static_cast<float>(dimension_ / 2) - 2);
                            window.draw(rec_bit);
                        }
                        else {
                            rec_bit.setPosition(bit_size*static_cast<float>(dimension_ + counter + 1),bit_size* static_cast<float>(dimension_ / 2 - graph_out_quarantine_[counter].num_r) - 2);
                            window.draw(rec_bit);
                        }
                        if (graph_in_quarantine_[counter].num_i <= 1) {
                            inf_bit.setPosition(bit_size*static_cast<float>(dimension_ + counter),bit_size* static_cast<float>(dimension_) - 2);
                            window.draw(inf_bit);
                        }
                        else {
                            inf_bit.setPosition(bit_size*static_cast<float>(dimension_ + counter + 1),bit_size* static_cast<float>(dimension_) - graph_in_quarantine_[counter].num_i + 1);
                            window.draw(inf_bit);
                        }
                        if (graph_in_quarantine_[counter].num_s <= 1) {
                            sus_bit.setPosition(bit_size*static_cast<float>(dimension_ + counter),bit_size* static_cast<float>(dimension_) - 1);
                            window.draw(sus_bit);
                        }
                        else {
                            sus_bit.setPosition(bit_size*static_cast<float>(dimension_ + counter + 1),bit_size* static_cast<float>(dimension_ - graph_in_quarantine_[counter].num_s + 2));
                            window.draw(sus_bit);
                        }
                        if (graph_in_quarantine_[counter].num_r <= 1) {
                            rec_bit.setPosition(bit_size*static_cast<float>(dimension_ + counter),bit_size*static_cast<float>(dimension_) - 1);
                            window.draw(rec_bit);
                        }
                        else {
                            rec_bit.setPosition(bit_size*static_cast<float>(dimension_ + counter + 1),bit_size*static_cast<float>(dimension_ - graph_in_quarantine_[counter].num_r) + 1);
                            window.draw(rec_bit);
                        }
                ...
                for (int counter = 0; counter != (int)graph_out_quarantine_.size(); ++counter) {
                    if (graph_out_quarantine_[counter].num_i <= 1) {
                        ...
                        inf_bit.setPosition(bit_size*static_cast<float>(dimension_ + counter + 1),bit_size* static_cast<float>(dimension_ - graph_out_quarantine_[counter].num_i) + 1);
                        window.draw(inf_bit);
                    }
                    if (graph_out_quarantine_[counter].num_s <= 1) {
                        sus_bit.setPosition(bit_size*static_cast<float>(dimension_ + counter),bit_size*static_cast<float>(dimension_) - 1);
                        ...
                        sus_bit.setPosition(bit_size*static_cast<float>(dimension_ + counter + 1),bit_size* static_cast<float>(dimension_ - graph_out_quarantine_[counter].num_s) + 1);
                        window.draw(sus_bit);
                    }
                    if (graph_out_quarantine_[counter].num_r <= 1) {
                        rec_bit.setPosition(bit_size*static_cast<float>(dimension_ + counter), bit_size*static_cast<float>(dimension_) - 1);
                        ...
                        rec_bit.setPosition(bit_size*static_cast<float>(dimension_ + counter + 1),bit_size*static_cast<float>(dimension_ - graph_out_quarantine_[counter].num_r) + 1);
                        window.draw(rec_bit);
                    }
                }
            }
